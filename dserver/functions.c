...
void *sending(void *socket) {
   ...
   // ssize_t ret = 0;
    ...
    test=false;
    pthread_attr_t attr;
   if((ret=pthread_mutex_init(&mutex_jiff,NULL))!=0) {
       errno=ret;
    ...
    if((ret=pthread_mutex_init(&mutex_send,NULL))!=0) {
        errno=ret;
        perror("pthread_mutex_init");
    ...
    if((ret=pthread_cond_init(&cpu_cond,NULL))!=0) {
        errno=ret;
...
ssize_t send_cpu_num(int socket,int cpu_number) {
    ...
    if ((ret = send(socket, &data, sizeof(Data), MSG_WAITALL))<=0) {

        printf("Error sending data\n return = %d\n %s\n", (int) ret, ret==0 ? "socket closed" : "other error");
